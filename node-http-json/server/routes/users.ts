...
router.get('/', function (req: express.Request, res: express.Response) {
    var dbres = Utils.getall(res.locals.authtoken);
    if (!dbres[0]) {
...
router.post('/', function (req: express.Request, res: express.Response) {
    ...
    var dbres = Utils.update(req.body, res.locals.authtoken);
    if (!dbres[0]) {
        res.status(dbres[1]).json({ error: dbres[2] });
    }
    else {
...
router.put('/', function (req: express.Request, res: express.Response) {
    ...
    var dbres = Utils.add(req.body, res.locals.authtoken);
    if (!dbres[0]) {
        res.status(dbres[1]).json({ error: dbres[2] });
    }
    else {
...
router.delete('/:username', function (req: express.Request, res: express.Response) {
    ...
    var dbres = Utils.remove(req.params.username, res.locals.authtoken);
    if (!dbres[0]) {
        res.status(dbres[1]).json({ error: dbres[2] });
    }
    else {
